...
// INPUT INITDATA
// ============================================================
echo "\033[36mPaste initData:\033[0m ";
$init = '';

// Coba baca dari pipe/stdin
$stdin = fopen('php://stdin', 'r');
if (!$stdin) die("Cannot open stdin.\n");

// Non-blocking check: ada input piped?
$read = [$stdin];
$write = $except = null;
...
// Kalau kosong (bukan pipe), tanya interactive
if (empty($init)) {
    echo "\033[36mPaste initData:\033[0m ";
    $init = trim(fgets(STDIN) ?: '');
    if (empty($init)) {
        // Fallback: baca dari argument --data=xxx
        foreach ($argv ?? [] as $a) {
...
if (empty($init)) die("\033[31mInitData kosong!\033[0m\n");

// Normalize: kalau user paste full URL, extract tgWebAppData
...
echo "\033[32m[OK]\033[0m InitData loaded (" . strlen($init) . " chars)\n";
...
function post(string $endpoint, array $extra = []): ?array {
    ...
    if ($raw === false) {
        ...
        L("  \033[31mNET:\033[0m " . substr($msg, 0, 100));
        return null;
    }
    ...
    if (!is_array($d)) {
        ...
        L("  \033[31mJSON:\033[0m HTTP {$code} | " . substr($raw, 0, 150));
        return null;
    }
...
while (true) {
    ...
    if (isset($resp['_error'])) {
        ...
        if (stripos($msg, 'auth') !== false || stripos($msg, 'invalid') !== false || stripos($msg, 'expired') !== false) {
            L("  \033[31mInitData expired! Bot stop.\033[0m");
            break;
        }
    ...
    if ($blocked) {
        L("  \033[31mBLOCKED!\033[0m " . ($user['ban_reason'] ?? ''));
        break;
    }

    L("  OK | Bal: \033[33m{$balance}\033[0m | CD: {$cooldown}s | Risk: {$risk}");
        ...
        if ($challenge) {
            ...
            if ($answer) {
                ...
                if ($v && ($v['status'] ?? '') === 'success') {
                    ...
                    L("  \033[32mCaptcha OK!\033[0m Valid {$vh}h");
                } else {
                    $em = $v['_error'] ?? ($v['detail'] ?? 'fail');
                    L("  \033[31mCaptcha fail: {$em}\033[0m");
                    zzz(30); $failed++; continue;
                }
            } else {
                L("  \033[31mCannot solve\033[0m");
                zzz(30); $failed++; continue;
            }
        } else {
            L("  \033[31mNo challenge\033[0m");
            zzz(30); $failed++; continue;
        }
    } else {
    ...
    if (empty($session_uid)) {
        L("  \033[31mNo session_uid\033[0m");
        $retries++; $failed++; zzz(10); continue;
    }
    ...
    L("  \033[32m+{$reward} sats\033[0m | Bal: \033[33m{$new_bal}\033[0m | CD: {$cd}s");
